...
app.post('/students', async (c) => {
  ...
  if (!data.aadharNumber) {
    return c.json({ error: 'Aadhar number is required' }, 400);
  }

  const proctorExists = await prisma.professor.findUnique({
    where: { id: data.proctorId },
  });

  if (!proctorExists) {
    return c.json({ error: 'Invalid proctorId: Professor does not exist' }, 400);
  }
